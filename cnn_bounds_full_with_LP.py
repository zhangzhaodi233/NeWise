...
def warmup(model, x, eps_0, p_n, activation, func):
    print('Warming up...')
    ...
    shapes.append((1,1,1))
    func(weights, biases, shapes, model.pads, model.strides, model.sizes, model.types, x, eps_0, p_n, activation)
...
def run_certified_bounds(file_name, n_samples, p_n, q_n, data_from_local=True, method='NeWise', 
                         cnn_cert_model=False, vnn_comp_model=False, 
                         eran_fnn=False, eran_cnn=False,
                         activation = 'sigmoid', mnist=False, cifar=False, fashion_mnist=False, gtsrb=False, log_name=timestr):
    np.random.seed(1215)
    random.seed(1215)
    ...
    warmup(model, inputs[0].astype(np.float32), eps_0, p_n, activation, find_output_bounds)
    ...
    for i in range(len(inputs)):
        ...
        for j in range(steps):
            LB_total, UB_total, _, _, _, _, _, _ = find_output_bounds(model.weights, model.biases, model.shapes, model.pads, model.strides, model.sizes, model.types, inputs[i].astype(np.float32), np.exp(log_eps), p_n, activation, method)
