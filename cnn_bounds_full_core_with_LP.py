...
def run_certified_bounds_core(file_name, n_samples, p_n, q_n, data_from_local=True, method='NeWise', cnn_cert_model=False, activation = 'sigmoid', mnist=False, cifar=False, fashion_mnist=False, gtsrb=False, log_name=timestr):
    np.random.seed(1215)
    random.seed(1215)
    if activation == 'atan':
